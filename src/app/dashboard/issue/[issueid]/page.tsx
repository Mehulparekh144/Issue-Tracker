...
function IssuePage({ params }: PageProps) {
  const UserToolTip = dynamic(
    () => import("@/components/dashboard/UserToolTip"),
    {
      suspense: true,
    }
  );
  ...
  const ImagesModal = dynamic(
    () => import("@/components/dashboard/ImagesModal"),
    {
      suspense: true,
    }
  );
                    ...
                    <DropdownMenu>
                      <DropdownMenuTrigger>
                        <Button
                          ...
                          } hover:bg-${priorityObject[data.priority]}`}
                        >
                      ...
                      <DropdownMenuContent className="flex flex-col gap-1 items-center justify-start">
                        {Object.keys(priorityObject)
                          ...
                          .map((item, index) => (
                            <Button
                              onClick={() =>
                                changePriorityHandler(issueid, item)
                              }
                              key={index}
                              size={"sm"}
